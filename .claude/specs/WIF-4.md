...
- [x] Step 18: Add flags toggle to map settings — add `FLAGS_VISIBLE` runtime state in `src/Map/Map.jsx` (default true, persisted via `STORAGE_KEY_FLAGS` in `src/config.js`); toggle a `map--flags-hidden` CSS class on the map container that sets `.flag { display: none }`; add a toggle row to `MapSettings` using a new `flagsLabel` translation key in both EN and ES (`src/Common/translations.js`).
- [x] Step 19: Add mobile bottom sheet sidebar — add `position: fixed; bottom: var(--footer-height)` and `max-height: 65svh` under `respond-max(767)` in `src/InfoSidebar/InfoSidebar.scss`; add `@keyframes slide-up-sheet` for the open animation; round top corners only.
