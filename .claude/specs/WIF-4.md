...
- [ ] Step 14: Add clustering toggle to map settings — move the `CLUSTERING_ENABLED` flag from a static build constant to a runtime state in `src/Map/Map.jsx`; persist in `localStorage` via `STORAGE_KEY_CLUSTERING` (config constant as default); add toggle row to `MapSettings` panel.
- [ ] Step 15: Add fullscreen toggle to map settings — add runtime `fullscreenEnabled` state in `src/Map/Map.jsx`; use browser Fullscreen API (`requestFullscreen` / `exitFullscreen`) triggered directly from the user gesture; sync state on `fullscreenchange` event (Escape key); persist in `localStorage` via `STORAGE_KEY_FULLSCREEN` (config constant as default); add toggle row to `MapSettings` panel.
- [ ] Step 16: Add pane z-index layering — use react-leaflet `<Pane>` in `src/Map/Map.jsx` to explicitly assign GeoJSON polygons to a `polygons` pane (z-index 200) and markers to a `markers` pane (z-index 400).
- [ ] Step 17: Add mobile bottom sheet sidebar — add responsive CSS in `src/GlobalStyles/Components/_sidebars.scss` for viewports below 768px to position the sidebar as a bottom drawer; add slide-up animation.
- [ ] Step 18: Add minimap — install `leaflet-minimap`; mount inside `<MapContainer>` in `src/Map/Map.jsx` on desktop only (hide below 768px viewport); guard behind config flag.
- [ ] Step 19: Add smooth wheel zoom — install `@luomus/leaflet-smooth-wheel-zoom`; integrate in `src/Map/Map.jsx` behind `SMOOTH_ZOOM_ENABLED` flag; disable default `scrollWheelZoom` when active.
- [ ] Step 20: Add reduced motion support — add `@media (prefers-reduced-motion: reduce)` rules to `src/CustomMarker/CustomMarker.scss` and `src/GlobalStyles/Components/_sidebars.scss`; pass `animate: false` to Leaflet `setView`/`flyTo` calls when the media query matches; guard behind `REDUCED_MOTION_ENABLED` flag.
