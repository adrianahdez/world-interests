...
- [x] Step 7: Add gesture handling — install `leaflet-gesture-handling`; integrate in `src/Map/Map.jsx` behind `GESTURE_HANDLING_ENABLED` flag from config; coordinate with existing `scrollWheelZoom` setting.
- [x] Step 8: Add no-data country indicator — refactor `src/Map/geoJsonConfig.js` `setConfig` into a factory that receives the `data` object; apply desaturated/dashed style to countries with no data in the current category; guard behind `NO_DATA_INDICATOR_ENABLED` flag. Also added `COUNTRY_HOVER_LABEL_ENABLED` flag and `HoverCountryLabel` component that shows the hovered country name at the bottom-right of the map via direct DOM update (no re-render).
- [x] Step 9: Implement API retry with exponential backoff — replace the current single `getData` call in `src/Map/Map.jsx` with a retry wrapper (max 3 retries, 2s/4s/8s delays, capped at 30s); show a subtle "retrying…" indicator in the map error overlay.
- [x] Step 10: Add marker clustering — install `leaflet.markercluster` (or `react-leaflet-cluster`); wrap markers in `<MarkerClusterGroup>` in `src/Map/Map.jsx` behind `CLUSTERING_ENABLED` constant; style cluster icons to match the app theme.
- [x] Step 11: Add ARIA labels — add `aria-label` to the map container in `src/Map/Map.jsx` and to each marker's DOM element in `src/CustomMarker/CustomMarker.jsx`; add `role="region"` to the map wrapper. Also fixed a bug where switching language caused stats-based pin styling (padding/opacity/colour) to be lost by memoising marker positions so `CustomMarker` effects don't re-fire on language re-renders.
- [x] Step 12: Add keyboard navigation for markers — add `tabIndex={0}` and `keydown` (Enter/Space) handler to each marker in `src/CustomMarker/CustomMarker.jsx` to open the sidebar.
- [x] Step 13: Add heatmap overlay — install `@linkurious/leaflet-heat`; add a toggleable `<HeatmapLayer>` inside `<MapContainer>` in `src/Map/Map.jsx` driven by `HEATMAP_ENABLED` and a UI toggle button; guard behind config flag. Added reusable `MapSettings` panel (⚙ button, bottom-left) with a modern pill toggle; panel is theme-aware and mobile-friendly.
- [x] Step 14: Add clustering toggle to map settings — move the `CLUSTERING_ENABLED` flag from a static build constant to a runtime state in `src/Map/Map.jsx`; persist in `localStorage` via `STORAGE_KEY_CLUSTERING` (config constant as default); add toggle row to `MapSettings` panel.
- [x] Step 15: Add fullscreen toggle to map settings — add runtime `fullscreenEnabled` state in `src/Map/Map.jsx`; use browser Fullscreen API (`requestFullscreen` / `exitFullscreen`) triggered directly from the user gesture; sync state on `fullscreenchange` event (Escape key); fullscreen intentionally NOT persisted in localStorage (browser blocks auto-enter on load); add toggle row to `MapSettings` panel.
- [x] Step 16: Add pane z-index layering — use react-leaflet `<Pane>` in `src/Map/Map.jsx` to explicitly assign GeoJSON polygons to a `polygons` pane (z-index 200) and markers to a `markers` pane (z-index 400).
- [x] Step 17: Fix category display name in InfoSidebar — `InfoSidebar` currently shows the raw category slug (e.g. `"music"`); lift `categoryName` state to `src/App/App.jsx`, have `src/Categories/Categories.jsx` emit the display name on selection and on initial load, and pass `categoryName` to `InfoSidebar` instead of `category`.
- [x] Step 18: Make category name fully language-aware — replace the `categoryName` string state in `src/App/App.jsx` with a `categoriesMap` (slug → display name for the current language); `Categories` emits the full map via `onCategoriesLoad` callback after each fetch; `categoryName` is derived as `categoriesMap[category] || category`, giving instant language-switch updates and a non-empty slug fallback until the API responds. Remove the now-redundant `onCategoryNameChange` call from the click handler.
- [ ] Step 19: Add mobile bottom sheet sidebar — add responsive CSS in `src/GlobalStyles/Components/_sidebars.scss` for viewports below 768px to position the sidebar as a bottom drawer; add slide-up animation.
- [ ] Step 20: Add minimap — install `leaflet-minimap`; mount inside `<MapContainer>` in `src/Map/Map.jsx` on desktop only (hide below 768px viewport); guard behind config flag.
- [ ] Step 21: Add smooth wheel zoom — install `@luomus/leaflet-smooth-wheel-zoom`; integrate in `src/Map/Map.jsx` behind `SMOOTH_ZOOM_ENABLED` flag; disable default `scrollWheelZoom` when active.
- [ ] Step 22: Add reduced motion support — add `@media (prefers-reduced-motion: reduce)` rules to `src/CustomMarker/CustomMarker.scss` and `src/GlobalStyles/Components/_sidebars.scss`; pass `animate: false` to Leaflet `setView`/`flyTo` calls when the media query matches; guard behind `REDUCED_MOTION_ENABLED` flag.
