...
- [x] Step 20: Add reduced motion support — add `@media (prefers-reduced-motion: reduce)` rules to `src/CustomMarker/CustomMarker.scss` and `src/InfoSidebar/InfoSidebar.scss` to disable pin hover transforms and sidebar animations; guard Leaflet pan/fly calls behind `REDUCED_MOTION_ENABLED` flag in `src/config.js`.
