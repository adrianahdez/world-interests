...
- [ ] Step 19: Add minimap — install `leaflet-minimap`; mount inside `<MapContainer>` in `src/Map/Map.jsx` on desktop only (hide below 768px viewport); guard behind config flag.
- [ ] Step 20: Add smooth wheel zoom — install `@luomus/leaflet-smooth-wheel-zoom`; integrate in `src/Map/Map.jsx` behind `SMOOTH_ZOOM_ENABLED` flag; disable default `scrollWheelZoom` when active.
- [ ] Step 21: Add reduced motion support — add `@media (prefers-reduced-motion: reduce)` rules to `src/CustomMarker/CustomMarker.scss` and `src/GlobalStyles/Components/_sidebars.scss`; pass `animate: false` to Leaflet `setView`/`flyTo` calls when the media query matches; guard behind `REDUCED_MOTION_ENABLED` flag.
