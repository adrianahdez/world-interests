...
- [x] Step 3: Thread normalization data through `src/Map/Map.jsx` — compute `minViews`/`maxViews` from `data` before the `processPoint` loop; pass them into `processPoint` (and down to `setUpPointAttributes`).
- [x] Step 4: Add zoom-responsive behaviour — extend `MapViewSaver` in `src/Map/Map.jsx` to toggle `.map--low-zoom` on the `.map-container` element on `zoomend` (and on mount); add CSS rules in the marker stylesheet for `scale(0.65)` and flag `display: none` under `.map--low-zoom`.
- [x] Step 5: Replace algorithmic color generation with a curated palette in `src/Map/Points/Points.js` — define ~20 visually distinct colors; replace `convertToHex` with a deterministic name hash that maps any channel name to a palette index; same name always gets the same color, different names spread across the full palette.
- [x] Step 6: Add stat-based brightness via CSS variable `--pin-brightness` on `.custom-marker__bg` and `.custom-marker__bg-pointer`; range 0.55 (low views) → 1.15 (high views, slight shine); previous static value was 0.75; hover CSS `brightness(0.5)` preserved via specificity.
- [x] Step 7: Manual QA — verify all acceptance criteria: stat-based pin appearance (opacity, border thickness, brightness), zoom-responsive sizing and flag visibility, distinct pin colors, no console errors.
