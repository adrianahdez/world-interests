...
- [ ] Step 4: Add zoom-responsive behaviour — extend `MapViewSaver` in `src/Map/Map.jsx` to toggle `.map--low-zoom` on the `.map-container` element on `zoomend` (and on mount); add CSS rules in the marker stylesheet for `scale(0.65)` and flag `display: none` under `.map--low-zoom`.
