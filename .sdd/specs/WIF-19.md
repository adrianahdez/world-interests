...
- [x] Paso 38 (backend — desempate por recencia en histórico de canales): En `src/Repositories/TrendingHistoryRepo.php`, cambiar el desempate del orden de `getCountryHistoryChannels` de `max_views DESC` a `MAX(captured_at) DESC`. Razonamiento: dos canales con los mismos días en #1 — el que fue #1 más recientemente es más relevante para una app de tendencias. El desempate por vistas favorecía implícitamente a canales grandes con historial acumulado; la recencia es neutral y más alineada con "trending activo".
