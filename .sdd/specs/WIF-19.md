...
- [x] Paso 42 (frontend — integración): En `src/CountryPanel/CountryPanel.jsx`, importar `InfoTooltip` e `IconInfo`. En `HistoricalTab`, añadir `<InfoTooltip text={mode === 'videos' ? tr.historicalInfoVideos : tr.historicalInfoChannels}><IconInfo /></InfoTooltip>` dentro de `country-panel__subtabs`, separado del tablist (no dentro del `role="tablist"` para no romper la accesibilidad). En `CountryPanel.scss`, hacer que el wrapper del sub-toggle use `display: flex; align-items: center; justify-content: space-between` para empujar el icono a la derecha.
- [x] Paso 43: QA manual — verificar tooltip en ambas subtabs (Vídeos / Canales), ambos idiomas, ambos temas; confirmar que el texto cambia al alternar el modo; comprobar que no desborda el panel en pantallas estrechas; confirmar que `AppearancesTooltip` existente no se ve afectado. En `src/Repositories/TrendingHistoryRepo.php`, cambiar el desempate de `getCountryHistoryChannels` y `getCountryHistoryVideos` de vistas a `MAX(captured_at) DESC` (recencia). Razonamiento: dos entradas con los mismos días en #1 — la más reciente es más relevante para una app de tendencias. Para vídeos el argumento es aún más fuerte: son efímeros por naturaleza y `view_count` acumulado favorecería siempre a contenido antiguo sobre uno que acaba de explotar.
- [x] Paso 44 (frontend — limpieza de nomenclatura): En `src/Common/translations.js`, renombrar `mostViewedTooltip` → `topCountriesTooltip` y `worldwideNumberOne` → `topCountriesLabel` (EN + ES). Actualizar sus referencias en `src/Map/Map.jsx`. En `src/Map/Countries/Countries.scss`, renombrar la clase CSS `.map-overlay-label--most-viewed` → `.map-overlay-label--top-countries` (selector BEM + override del tema light). Actualizar el `className` en `Map.jsx` y los comentarios en ambos archivos y en `src/config.js`. Actualizar además el texto de `historicalInfoVideos` / `historicalInfoChannels` (EN + ES) al acordado: «Ordered by number of days at #1. Ties: most recent #1 appearance goes first.» / «Ordenado por cantidad de días como #1. Empates: aparición más reciente va primero.»
- [x] Paso 45 (frontend — fecha + atributo de control en LiveTimestamp): En `src/LiveTimestamp/LiveTimestamp.jsx`, añadir un segundo `Intl.DateTimeFormat` (`dateFmt`, `{ month: 'short', day: 'numeric' }`) cacheado con `useMemo`. Mostrar `{label} {date}, {time}` en lugar de solo `{label} {time}`. Aceptar prop opcional `capturedAt: string` y escribirla como `data-mark={capturedAt || undefined}` en el `<span>` — el atributo solo aparece cuando el dato ha cargado, y su nombre es intencionalmente opaco. En `src/CountryPanel/CountryPanel.jsx` (`RealtimeTab`), pasar `capturedAt={data?.latest_capture_at}`, que proviene del campo homónimo devuelto por `GET /api/country/today`.
