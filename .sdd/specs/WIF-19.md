...
- [ ] Step 19: Manual QA per CLAUDE.md for the historical split — both sub-toggle lists (videos default, channels compact), date tooltip order (Step 14), worldwide-#1 label (Step 15), no `favorite_count` regressions (Step 16), both languages/themes, mobile layout. Confirm real-time tab and `/api/trending` unaffected.
